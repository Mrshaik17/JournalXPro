...
import { TrendingUp, Target, BarChart3, Activity, Wallet, Eye, EyeOff, ArrowUpRight, ArrowDownRight } from "lucide-react";
...
const Dashboard = () => {
  ...
  const maxProfit = profitTrades.length > 0 ? Math.max(...profitTrades.map((t) => Number(t.pnl_amount))) : 0;
  const maxLoss = losses > 0 ? Math.min(...trades.filter((t) => Number(t.pnl_amount) < 0).map((t) => Number(t.pnl_amount))) : 0;
  ...
  const stats = [
    ...
    { label: "Total P&L", value: hideProfit ? "•••••" : `$${totalPnl.toFixed(2)}`, icon: TrendingUp, color: totalPnl >= 0 ? "text-success" : "text-destructive" },
    ...
    { label: "Avg Profit", value: hideProfit ? "•••••" : `$${avgProfit.toFixed(2)}`, icon: ArrowUpRight, color: "text-success" },
    { label: "Max Profit", value: hideProfit ? "•••••" : `$${maxProfit.toFixed(2)}`, icon: ArrowUpRight, color: "text-success" },
    { label: "Max Loss", value: hideProfit ? "•••••" : `$${maxLoss.toFixed(2)}`, icon: ArrowDownRight, color: "text-destructive" },
    { label: "Total Accounts", value: hideAccounts ? "•••" : String(accounts.length), icon: Wallet, color: "text-primary" },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setHideProfit(!hideProfit)} className="text-muted-foreground hover:text-foreground transition-colors p-2 rounded-md hover:bg-card" title="Toggle profit visibility">
            {hideProfit ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
          </button>
          <button onClick={() => setHideAccounts(!hideAccounts)} className="text-muted-foreground hover:text-foreground transition-colors p-2 rounded-md hover:bg-card" title="Toggle accounts visibility">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {accounts.map((acc, i) => {
                ...
                return (
                  ...
                  >
                    <div className="text-sm text-muted-foreground mb-1 group-hover:text-primary transition-colors">{acc.name}</div>
                    <div className="font-mono text-xl font-bold">{hideProfit ? "•••••" : `$${Number(acc.current_balance).toFixed(2)}`}</div>
                    <div className={`font-mono text-sm mt-1 ${pnl >= 0 ? "text-success" : "text-destructive"}`}>
                      {hideProfit ? "•••" : `${pnl >= 0 ? "+" : ""}${pnlPercent}% ($${pnl.toFixed(2)})`}
                    </div>
                  </motion.div>
                );
              })}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {recentTrades.map((trade) => (
                  <tr key={trade.id} className="border-b border-border last:border-0 hover:bg-muted/5 transition-colors">
                    ...
                    <td className={`p-3 text-right font-mono ${hideProfit ? "" : Number(trade.pnl_amount) >= 0 ? "text-success" : "text-destructive"}`}>
                      {hideProfit ? "•••" : `${Number(trade.pnl_amount) >= 0 ? "+" : ""}$${Number(trade.pnl_amount).toFixed(2)}`}
                    </td>
