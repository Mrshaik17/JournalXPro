...
import { Check, Crown, Zap, Upload, Copy } from "lucide-react";
...
const plans = [
  {
    ...
    name: "Pro",
    icon: Zap,
    features: ["100 trades/month", "Advanced analytics", "All trading pairs", "Priority support"],
  },
  {
    id: "pro_plus",
    name: "Pro+",
    icon: Crown,
    features: ["Unlimited trades", "AI trade analysis", "Custom fields", "Export to Excel", "Dedicated support"],
  },
];
...
const Upgrade = () => {
  ...
  const submitPayment = useMutation({
    mutationFn: async () => {
      ...
      const amount = selectedPlan === "pro" ? (pricing.pro || 9) : (pricing.pro_plus || 19);
    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Current plan: <span className="text-primary font-semibold capitalize">{profile?.plan || "free"}</span>
        </p>
      </div>

      {hasPendingPayment && (
        <div className="rounded-lg border border-yellow-500/30 bg-yellow-500/5 p-4">
          <p className="text-sm text-yellow-400">⏳ You have a pending payment awaiting approval. You'll be upgraded once it's verified.</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {plans.map((plan) => {
          const price = plan.id === "pro" ? (pricing.pro || 9) : (pricing.pro_plus || 19);
          ...
          return (
            <button
              ...
              className={`text-left rounded-lg border p-5 transition-all ${
                isSelected
                  ? "border-primary divine-glow bg-primary/5"
                  : isCurrent
      ...
      {selectedPlan && !hasPendingPayment && (
        <div className="rounded-lg border border-border bg-card p-6 space-y-5">
          ...
          <div className="rounded-md border border-border bg-background p-4 space-y-2">
            ...
            {paymentMethod === "upi" && paymentSettings.upi_id && (
              <div className="flex items-center justify-between">
                <span className="font-mono text-sm">{paymentSettings.upi_id}</span>
                <Button size="sm" variant="ghost" onClick={() => copyToClipboard(paymentSettings.upi_id)}>
            ...
            {paymentMethod === "gpay" && paymentSettings.phone_number && (
              <div className="flex items-center justify-between">
                <span className="font-mono text-sm">{paymentSettings.phone_number}</span>
                <Button size="sm" variant="ghost" onClick={() => copyToClipboard(paymentSettings.phone_number)}>
                  <Copy className="h-3.5 w-3.5" />
                </Button>
              </div>
            )}
            {paymentMethod === "crypto" && paymentSettings.crypto_wallet && (
              <div className="flex items-center justify-between">
                <span className="font-mono text-sm text-xs break-all">{paymentSettings.crypto_wallet}</span>
                <Button size="sm" variant="ghost" onClick={() => copyToClipboard(paymentSettings.crypto_wallet)}>
                  <Copy className="h-3.5 w-3.5" />
                </Button>
              </div>
            )}
          ...
          {/* Transaction ID */}
          <div>
            <label className="text-xs text-muted-foreground block mb-1">Transaction / Reference ID</label>
            <Input
              value={transactionId}
          ...
          <Button
            ...
            disabled={submitPayment.isPending || uploading || !transactionId}
            className="w-full bg-primary text-primary-foreground font-semibold"
          >
            {submitPayment.isPending || uploading ? "Submitting..." : `Submit Payment for ${selectedPlan === "pro" ? "Pro" : "Pro+"}`}
          </Button>
        </div>
      )}
