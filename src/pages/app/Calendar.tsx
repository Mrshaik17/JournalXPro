...
const Calendar = () => {
  ...
  const { data: trades = [] } = useQuery({
    queryKey: ["trades-calendar", user?.id],
    queryFn: async () => {
  ...
  const getTradesForDate = (date: Date) => trades.filter((t: any) => isSameDay(new Date(t.created_at), date));
  const getDayPnl = (date: Date) => {
    const dayTrades = getTradesForDate(date);
    if (dayTrades.length === 0) return null;
    return dayTrades.reduce((s: number, t: any) => s + Number(t.pnl_amount), 0);
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Daily P&L overview & market news</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {paddedDays.map((day, i) => {
              ...
              return (
                <button
                  ...
                  className={`relative p-2 rounded-md text-sm transition-all min-h-[60px] flex flex-col items-center ${
                ...
                >
                  ...
                  {dayPnl !== null && (
                    <span className={`text-[9px] font-mono mt-0.5 ${dayPnl >= 0 ? "text-success" : "text-destructive"}`}>
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          ...
          {selectedDate && selectedDayPnl !== null && (
            <div className={`mb-4 p-3 rounded border ${selectedDayPnl >= 0 ? "border-success/20 bg-success/5" : "border-destructive/20 bg-destructive/5"}`}>
              <div className="text-xs text-muted-foreground">Day P&L</div>
              <div className={`font-mono text-lg font-bold ${selectedDayPnl >= 0 ? "text-success" : "text-destructive"}`}>
                {selectedDayPnl >= 0 ? "+" : ""}${selectedDayPnl.toFixed(2)}
              </div>
              <div className="text-xs text-muted-foreground">{selectedDayTrades.length} trade{selectedDayTrades.length !== 1 ? "s" : ""}</div>
            </div>
          )}
              ...
              {selectedNews.length > 0 ? selectedNews.map((n: any) => (
                <div key={n.id} className="p-3 rounded border border-border bg-background">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {n.asset_name && <span className="text-[10px] font-mono bg-primary/10 text-primary px-1.5 py-0.5 rounded">{n.asset_name}</span>}
                  </div>
