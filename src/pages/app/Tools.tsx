...
import { getPlanAccess, getEffectivePlan } from "@/lib/planAccess";
...
const Tools = () => {
  ...
  const fetchPlan = async () => {
    ...
    const { data, error } = await supabase
      .from("profiles") // ✅ FIXED TABLE
      .select("plan, plan_expiry") // ✅ ADDED plan_expiry
      .eq("id", user.id) // ⚠️ IMPORTANT (check column name)
      .maybeSingle();
    ...
    if (data) {
  console.log("DB PLAN 👉", data.plan);
  console.log("EXPIRY 👉", data.plan_expires_at);
  ...
  const plan = userPlanRaw; // getEffectivePlan(userPlanRaw, planExpiry); // ✅ REPLACE WITH THIS TO ENFORCE EXPIRY
  console.log("FoRMATTED PLAN 👉", plan);
  const access = getPlanAccess(plan);
  // 🔥 ADD THESE 3 LINES BELOW
console.log("USER OBJECT 👉", user);
console.log("RAW PLAN 👉", userPlanRaw);
console.log("FINAL PLAN 👉", plan);
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="lot" className="space-y-5">
            ...
            {canUseAllTools ? null : (
              <div className="absolute inset-0 z-10 flex items-center justify-center bg-background/80 backdrop-blur-sm rounded-xl">
                <div className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Unlock this tool</p>
                </div>
              </div>
            )}
