...
import { User, CreditCard, Palette, LogOut, Crown, Shield, Trash2, AlertTriangle, MapPin, Clock, Rocket, Eye, EyeOff, Download } from "lucide-react";
...
const AppSettings = () => {
  ...
  useEffect(() => {
    if (profile) {
      ...
      // Load billing from profile custom data via site_settings user billing
    }
  }, [profile]);
  ...
  useEffect(() => {
    if (siteSettings.length > 0 && user) {
      ...
      if (billing.name) {
        ...
        setBillingDistrict(billing.district || "");
  ...
  const upsertUserSetting = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["site-settings"] }); toast.success("Saved!"); },
    onError: (err: any) => toast.error(err.message),
  });
  ...
  const saveBilling = () => {
    if (!billingName || !billingEmail || !billingPhone || !billingStreet || !billingCity || !billingState || !billingCountry || !billingPincode) {
  ...
  const saveTimezone = () => {
    upsertUserSetting.mutate({ key: `timezone_${user!.id}`, value: { timezone } });
  };
  ...
  const exportUserData = (format: "pdf" | "excel") => {
    if (format === "pdf") {
      const doc = new jsPDF();
      doc.setFontSize(16); doc.text("My Trading Data", 14, 20);
      doc.setFontSize(10); doc.text(`Generated: ${new Date().toLocaleDateString()}`, 14, 28);
      autoTable(doc, { startY: 36, head: [["Account", "Balance", "P&L"]], body: accounts.map((a: any) => [a.name, `$${Number(a.current_balance).toFixed(2)}`, `$${(Number(a.current_balance) - Number(a.initial_balance)).toFixed(2)}`]) });
      const fy = (doc as any).lastAutoTable?.finalY || 50;
      autoTable(doc, { startY: fy + 10, head: [["Date", "Pair", "Result", "P&L"]], body: trades.slice(0, 100).map((t: any) => [new Date(t.created_at).toLocaleDateString(), t.pair || "—", t.result || "—", `$${Number(t.pnl_amount).toFixed(2)}`]) });
      doc.save("my-trading-data.pdf");
      toast.success("PDF exported!");
    } else {
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(accounts.map((a: any) => ({ Account: a.name, Type: a.account_type, Balance: a.current_balance, Initial: a.initial_balance }))), "Accounts");
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(trades.map((t: any) => ({ Date: new Date(t.created_at).toLocaleDateString(), Pair: t.pair, Direction: t.direction, Result: t.result, PnL: t.pnl_amount, Pips: t.pips }))), "Trades");
  ...
  // Check if billing is saved
  const hasBilling = billingName && billingEmail && billingPhone && billingStreet && billingCity && billingState && billingCountry && billingPincode;
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {/* Billing reminder */}
      {!hasBilling && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-lg border border-yellow-500/30 bg-yellow-500/5 p-4 flex items-start gap-3">
          <AlertTriangle className="h-4 w-4 text-yellow-500 mt-0.5 shrink-0" />
          <div>
            <p className="text-sm text-foreground font-medium">Complete your billing address</p>
            <p className="text-xs text-muted-foreground">Billing details are required for subscription payments. Go to the Billing tab to save your details.</p>
          </div>
        </motion.div>
      )}
      ...
      <Tabs defaultValue="profile" className="space-y-4">
        <TabsList className="bg-card border border-border flex-wrap">
          ...
          <TabsTrigger value="billing"><MapPin className="h-3.5 w-3.5 mr-1.5" />Billing</TabsTrigger>
          ...
          <TabsTrigger value="advanced"><Rocket className="h-3.5 w-3.5 mr-1.5" />Advanced</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="billing">
          <div className="space-y-4">
            {/* Billing Address */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg border border-border bg-card p-6 space-y-4">
              <h3 className="font-semibold flex items-center gap-2"><MapPin className="h-4 w-4 text-primary" /> Billing Address</h3>
              <p className="text-xs text-muted-foreground">Required for subscription payments. All fields are mandatory.</p>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs text-muted-foreground block mb-1">Full Name *</label><Input value={billingName} onChange={(e) => setBillingName(e.target.value)} placeholder="John Doe" className="bg-background border-border" /></div>
                <div><label className="text-xs text-muted-foreground block mb-1">Email *</label><Input value={billingEmail} onChange={(e) => setBillingEmail(e.target.value)} placeholder="email@example.com" className="bg-background border-border" type="email" /></div>
                <div><label className="text-xs text-muted-foreground block mb-1">Phone Number *</label><Input value={billingPhone} onChange={(e) => setBillingPhone(e.target.value)} placeholder="+91..." className="bg-background border-border" /></div>
                <div><label className="text-xs text-muted-foreground block mb-1">PIN Code *</label><Input value={billingPincode} onChange={(e) => setBillingPincode(e.target.value)} placeholder="500001" className="bg-background border-border font-mono" /></div>
              </div>
              <div><label className="text-xs text-muted-foreground block mb-1">Street Address *</label><Input value={billingStreet} onChange={(e) => setBillingStreet(e.target.value)} placeholder="123 Main Street" className="bg-background border-border" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs text-muted-foreground block mb-1">City *</label><Input value={billingCity} onChange={(e) => setBillingCity(e.target.value)} placeholder="Hyderabad" className="bg-background border-border" /></div>
                <div><label className="text-xs text-muted-foreground block mb-1">District</label><Input value={billingDistrict} onChange={(e) => setBillingDistrict(e.target.value)} placeholder="Rangareddy" className="bg-background border-border" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs text-muted-foreground block mb-1">State *</label><Input value={billingState} onChange={(e) => setBillingState(e.target.value)} placeholder="Telangana" className="bg-background border-border" /></div>
                <div><label className="text-xs text-muted-foreground block mb-1">Country *</label><Input value={billingCountry} onChange={(e) => setBillingCountry(e.target.value)} placeholder="India" className="bg-background border-border" /></div>
              </div>
              <Button onClick={saveBilling} disabled={upsertUserSetting.isPending} className="bg-primary text-primary-foreground">{upsertUserSetting.isPending ? "Saving..." : "Save Billing Details"}</Button>
            </motion.div>
        ...
        <TabsContent value="preferences">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="rounded-lg border border-border bg-card p-6 space-y-4">
              <h3 className="font-semibold flex items-center gap-2"><Clock className="h-4 w-4 text-primary" /> Timezone</h3>
              <Select value={timezone} onValueChange={setTimezone}>
                <SelectTrigger className="bg-background border-border max-w-sm"><SelectValue /></SelectTrigger>
                <SelectContent className="bg-card border-border">
                  {timezones.map((tz) => <SelectItem key={tz} value={tz}>{tz}</SelectItem>)}
                </SelectContent>
              </Select>
              <Button onClick={saveTimezone} className="bg-primary text-primary-foreground">Save Timezone</Button>
            </div>
            {/* Export */}
            {trades.length > 0 && (
              <div className="rounded-lg border border-border bg-card p-6 space-y-4">
                <h3 className="font-semibold flex items-center gap-2"><Download className="h-4 w-4 text-primary" /> Export My Data</h3>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => exportUserData("pdf")}>Export PDF</Button>
                  <Button variant="outline" size="sm" onClick={() => exportUserData("excel")}>Export Excel</Button>
                </div>
              </div>
            )}
          </motion.div>
          ...
          <div className="space-y-4">
            <div className="rounded-lg border border-border bg-card p-6 space-y-4">
              ...
              <div className="relative">
                ...
                <Input type={showNewPw ? "text" : "password"} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Min 6 characters" className="bg-background border-border max-w-sm pr-10" />
              ...
              {accounts.length > 0 ? (
                <div className="space-y-3">
                  <Select value={deleteAccountId} onValueChange={setDeleteAccountId}>
                    ...
                    <SelectContent className="bg-card border-border">
                      {accounts.map((a: any) => <SelectItem key={a.id} value={a.id}>{a.name} (${Number(a.current_balance).toFixed(0)})</SelectItem>)}
                    </SelectContent>
                  </Select>
                  {deleteAccountId && (
                    <div className="space-y-2">
        ...
        <TabsContent value="advanced">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            {[
              { title: "API Integrations", desc: "Connect your broker APIs for automatic trade sync" },
              { title: "Broker Sync", desc: "Sync trades directly from your broker platform" },
              { title: "Advanced Analytics", desc: "Deep performance analytics with AI-powered insights" },
            ].map((item) => (
              <div key={item.title} className="rounded-lg border border-border bg-card p-6 opacity-60">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-semibold flex items-center gap-2"><Rocket className="h-4 w-4 text-primary" /> {item.title}</h3>
                    <p className="text-sm text-muted-foreground mt-1">{item.desc}</p>
                  </div>
                  <span className="text-xs font-medium text-primary bg-primary/10 px-3 py-1 rounded-full">Coming Soon</span>
                </div>
              </div>
            ))}
          </motion.div>
