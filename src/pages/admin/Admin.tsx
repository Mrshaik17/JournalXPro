...
const AdminDashboard = ({ queryClient, activeSection, setActiveSection, collapsed, setCollapsed, pendingNotif, setPendingNotif, chatNotif, setChatNotif, handleAdminLogout }: any) => {
  ...
  const deleteUserMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["admin-profiles"] }); toast.success("User deleted."); },
    onError: (err: any) => toast.error(err.message),
  });
  ...
  const totalRevenue = payments.filter((p: any) => p.status === "approved").reduce((s: number, p: any) => s + Number(p.amount), 0);
  const thisWeekUsers = profiles.filter((p: any) => new Date(p.created_at) >= new Date(Date.now() - 7 * 86400000)).length;
    ...
    <div className="min-h-screen flex bg-background text-foreground">
      ...
      <motion.aside animate={{ width: collapsed ? 64 : 220 }} transition={{ duration: 0.2 }} className="fixed left-0 top-0 h-full bg-card border-r border-border z-50 flex flex-col">
        ...
        <div className="border-t border-border p-2 space-y-1">
          <button onClick={handleAdminLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm text-muted-foreground hover:text-destructive hover:bg-secondary transition-all">
